...
Статические UI-строки вынесены в Android `strings.xml`:

- `app/src/main/res/values/strings.xml`

Так как основной экран лежит в `feature/commonMain`, строки передаются в shared
UI через `CatalogStrings`:

- `feature/src/commonMain/kotlin/com/artem/korenyakin/internassignment03/feature/catalog/CatalogStrings.kt`

Это позволяет хранить Android-ресурсы в привычном месте и при этом не завязывать
shared UI на Android API напрямую.

### Почему каталог остаётся на английском

Локализация интерфейса и локализация самих данных каталога — это разные задачи.
Кнопки, заголовки и служебные сообщения можно переводить локально через
ресурсы. Но названия, описания и категории товаров приходят из `DummyJSON`
на английском языке.

Для автоматического перевода контента в реальном приложении уже нужен не только
`strings.xml`, а отдельный backend/proxy к translation API. Это нужно для:
...
Поэтому в текущем решении локализован статический UI-слой, а данные каталога
отображаются в оригинале API.
...
Будут добавлены после подготовки финальных скриншотов экрана каталога.

## Видео

Ссылка на видео с демонстрацией работы приложения будет добавлена после записи.
