...
test("returns localized catalog from repository", async () => {
  ...
  assert.equal(localizedCatalogReads, 1);
  assert.equal(catalog.categories[0]?.title, "Beauty RU");
  assert.equal(catalog.products[0]?.title, "Mascara RU");
});
...
test("returns localized product details from repository", async () => {
  ...
  assert.equal(localizedProductDetailReads, 1);
  assert.equal(productDetails.product.title, "Mascara RU");
  assert.equal(productDetails.reviews[0]?.reviewerName, "Emma Wilson");
});
...
test("throws when source catalog is not available yet", async () => {
  ...
  await assert.rejects(
    ...
    /Source catalog is not available yet/,
  );
});
